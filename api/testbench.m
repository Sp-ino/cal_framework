function [result_arr, metrics] = testbench(algorithm,...
    ...
    % - meas_data: list of string literals containing the names of the
    %   files from which measured data is loaded. It can be a function handle,
    %   in which case the function should return a cell array containing structs.
    %   The struct variables should contain at least the following fields:
    %   y (the target) and X1 (the linear design matrix) so that apply_model()
    %   can extract the (input, target) pairs from it.
